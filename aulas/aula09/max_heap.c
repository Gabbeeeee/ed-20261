...
void inserir(MaxHeap *raiz, int valor) {
    ...
    raiz->dados[raiz->tamanho] = valor; //insere no final
    raiz->tamanho++;
...
void down_heap(MaxHeap *raiz, int indice) {
    ...
    if (esquerda < raiz->tamanho && 
    raiz->dados[esquerda] > raiz->dados[maior]) 
    {
        maior = esquerda;
    }

    if (direita < raiz->tamanho && 
    raiz->dados[direita] > raiz->dados[maior]) 
    {
        maior = direita;
    }
